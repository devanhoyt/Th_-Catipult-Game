...
class TestRoom(arcade.View):
    ...
    def setup(self):
        ...
        self.background = arcade.load_texture("Assets/Background/Backgrounds/Grass.png")
        ...
        self.launch_list = arcade.SpriteList()
        ...
        self.create_objective(game_settings.get("SCREEN_WIDTH") - 600, game_settings.get("SCREEN_HEIGHT")*.1,scale=.20)
    ...
    def update(self, delta_time):
        ...
        for enemy in self.enemy_list:
            enemy.center_x = enemy.pymunk_shape.body.position.x
            enemy.center_y = enemy.pymunk_shape.body.position.y
            enemy.angle = math.degrees(enemy.pymunk_shape.body.angle)
        
            
       
        hit_list = arcade.check_for_collision_with_lists(self.launch_list, self.enemy_list)
    ...
    def create_rock(self,mass = 4.0,elasticity = .5,friction = 0.4,force = 300000):
        ...
        #self.launch_list = arcade.SpriteList()
        self.launch_list.append(sprite)
    ...
    def create_objective(self,x, y, scale = 1, elasticity = .2, friction = .5,hp=100):
            ...
            shape.friction = friction
            self.space.add(body, shape)
            #enemy = Enemy(shape,":resources:images/enemies/slimeBlock.png",scale)
            enemy = arcade.Sprite(":resources:images/enemies/slimeBlock.png",scale)
            enemy.hp = hp
    ...
    def create_topR(self, x, y, scale = 1, elasticity = .2, friction = .5):
            length = 125 * scale
            height = 125 * scale
            mass = 2
            moment = pymunk.moment_for_box(mass, (length, height))
            ...
            shape.friction = friction
            self.space.add(body, shape)
            sprite = BoxSprite(shape,":resources:images/tiles/stoneHill_right.png",scale)
            self.sprite_list.append(sprite)
    def create_topL(self, x, y, scale = 1, elasticity = .2, friction = .5):
            length = 125 * scale
            height = 125 * scale
            mass = 2
            moment = pymunk.moment_for_box(mass, (length, height))
            ...
            shape.friction = friction
            self.space.add(body, shape)
            sprite = BoxSprite(shape,":resources:images/tiles/stoneHill_left.png",scale)
            self.sprite_list.append(sprite)
